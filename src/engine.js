import {uid,weekday,monday,addDays,ROUTINES} from './data.js';
...
function freeRanges(shift,reservations,window,notBefore=0){
 const w=windowForShift(window,shift);w.start=Math.max(w.start,notBefore);
...
export function scheduleDay(state,day,{notBefore=0}={}){
 ...
 const work=tasks.filter(t=>t.status!=='done'&&t.status!=='in-progress'&&!t.locked).sort((a,b)=>{
  // Tour zoning is reserved before the automatic zone fill. Other time windows are constrained.
  const score=t=>(priorityRank[t.priority]??2)*100+(t.source==='tour'?-30:0)+(t.window?-20:0)+(t.kind==='zone'&&t.source==='routine'?15:0);
  return score(a)-score(b)||((a.window?.end||3000)-(b.window?.end||3000));
 });
 for(const t of work){
  const candidates=[];
  for(const s of day.shifts){
   ...
   const segments=allocate(freeRanges(s,reserved[s.personId],t.window,notBefore),remaining(t));
  ...
  else {t.unscheduledReason=t.assignee?'The selected person is unavailable or has insufficient time.':t.window?'No eligible shift has enough free time in this task’s window.':'Not enough eligible shift capacity.';}
 }
...
export function generateTasks(state,day){
 ...
 const add=options=>tasks.push(taskBase({source:'routine',...options}));
 ...
 if(settings.routines.rfid.enabled){
  for(const d of state.departments){const rf=settings.rfid[d.id];
   if(rf?.enabled ? rf.days.includes(dow) : dow===2)add({key:`scan:${d.id}:${day.date}`,title:`${rf?.enabled?'RFID scan':'Deep out scan'} · D${d.id}`,kind:rf?.enabled?'rfid':'outs',department:d.id,minutes:settings.routines.rfid.minutes,priority:'high'});
  }
 }
 if(settings.routines.topstock.enabled&&settings.topstockDays.includes(dow)){
  ...
  for(const a of pending.slice(0,Math.ceil(pending.length/daysLeft)))add({key:`topstock:${a.id}:${progress.start}`,title:`Top stock · ${a.label}`,kind:'topstock',aisleId:a.id,department:a.department,minutes:a.topMinutes||settings.routines.topstock.minutes,priority:'high'});
 }
 ...
 if(settings.routines.zone.enabled){
  ...
  sorted.forEach(a=>add({key:`zone:${a.id}:${day.date}`,title:`Zone · ${a.label}`,kind:'zone',aisleId:a.id,department:a.department,minutes:a.zoneMinutes||settings.routines.zone.minutes,window:{...settings.zoneWindow},priority:'low'}));
 }
 ...
 if(settings.routines.modular.enabled){for(const m of state.modulars.filter(m=>!m.completed&&(!m.dueDate||m.dueDate<=day.date))){add({key:`modular:${m.id}`,title:`Modular ${m.category} · ${m.description}`,kind:'modular',department:m.department,minutes:m.minutes,modularId:m.id,notes:`${m.sections} sections`,priority:'normal'});}}
 return tasks;
}
...
export function integrateTour(state,day,notes,notBefore=0){
 ...
 for(const note of notes){
  ...
  result.tasks.push(taskBase({...note,id:uid(),key:`tour:${note.id}`,source:'tour',window:note.kind==='zone'?{...state.settings.zoneWindow}:undefined}));
 }
...
export function manualPlacement(state,day,taskId,personId,start){
 ...
 const end=start+remaining(task),bounds=windowForShift(task.window,shift);
 const blocks=[...pauses(shift),...day.tasks.filter(t=>t.id!==taskId&&t.assignedTo===personId).flatMap(t=>t.segments||[])];
 if(!Number.isFinite(start)||start<bounds.start||end>bounds.end||blocks.some(b=>overlap(b,{start,end})))throw Error('That time overlaps another task, a break, or falls outside the shift or task window.');
