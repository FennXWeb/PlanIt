import {initialState,ROUTINES,weekday,PRIORITIES,routinePolicy} from './data.js';
...
export function validateState(raw){
 ...
 if(!list(raw.aisles,2000)||raw.aisles.some(a=>!isObj(a)||!str(a.id,100)||!str(a.department,20)||!raw.departments.some(d=>d.id===a.department)||!str(a.label,40)||(a.description!==undefined&&!str(a.description,500))||typeof a.topstock!=='boolean'||!num(a.zoneMinutes,5,480)||!num(a.topMinutes,5,480)))fail();
 ...
 if(!list(raw.repeats,1000)||raw.repeats.some(r=>!isObj(r)||!str(r.id,100)||!str(r.title,300)||!num(r.minutes,5,960)||!str(r.department,20)||!str(r.assignee,100)||!['urgent','high','normal','low'].includes(r.priority)||!days(r.days)||typeof r.active!=='boolean'||!limitsValid(r)||(r.window&&!windowValid(r.window))))fail();
 ...
 // Older v1 workspaces and backups gain defaults without changing task history.
 const result=structuredClone(raw);
 result.aisles.forEach(a=>a.description??='');
 result.settings.routineSort??='priority';
 for(const r of ROUTINES)Object.assign(result.settings.routines[r.id],routinePolicy(result.settings,r.id));
 return result;
}
